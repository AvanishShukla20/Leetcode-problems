...
public:
    long long maximumCoins(vector<vector<int>>& coins, int k) {
        // Sort the coins based on their start positions
        ...
        for (int i = 0, j = 0; i < n; ++i) {
            ...
            if (j < n) {
                long long partial = (long long)(max(0, coins[i][0] + k - 1 - coins[j][0] + 1)) * coins[j][2];
        ...
        for (int i = 0, j = 0; i < n; ++i) {
            ...
            long long partial = (long long)(max(0, coins[i][1] - k - coins[j][0] + 1)) * coins[j][2];
            result = max(result, currentSum - partial);
        }
